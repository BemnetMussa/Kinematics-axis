...
\noindent\textbf{Lag band $[10,100]$ samples.}
...
\noindent\textbf{Movement floor $0.051$\,g.}
...
\noindent\textbf{Minimum cycles $3.5$ strides.}
...
mean, so that either factor being weak pulls confidence down.

\subsection*{Provenance --- where each number comes from}

Tags: \textsc{theory} = taken directly from the published method; \textsc{conv} = a
standard/physiological choice; \textsc{design} = an engineering construction.
...
\midrule
Regularity estimator & unbiased autocorr.\ peak & --- & Moe-Nilssen \& Helbostad (2004) trunk-gait index & \textsc{theory} \\
Lag band            & $[10,100]$  & --- & Stride periods $0.2$--$2.0$\,s at $50$\,Hz & \textsc{conv} \\
Movement floor      & $0.051$\,g  & --- & Chest $0.5$\,m/s$^2$ noise floor re-expressed in g (unit fix) & \textsc{design} \\
Min.\ cycles        & $3.5$ strides & --- & Gait-literature adequacy minimum ($\sim$3--4 strides) & \textsc{conv} \\
Prominence sat.\ ($P_\text{full}$) & $1.5$ & --- & Point at which the confidence term saturates & \textsc{design} \\
Confidence          & $\sqrt{c_\text{cyc}\,c_\text{prom}}$ & --- & Geometric mean of two adequacy factors & \textsc{design} \\
\bottomrule
\end{tabular}
\end{center}

\noindent The estimator is the one number taken straight from published theory; the score's
\emph{separation} was validated on PAMAP2/MotionSense (\S\,1.9), while the gates and
confidence are design choices calibrated to keep noise from scoring as rhythm. The
authoritative build lives in the \texttt{movement\_regularity.py} axis and the
convergence-test notebook.
...
Rhythmic-versus-irregular separation on pocket placement is confirmed only for the
still-versus-rhythmic contrast; irregular-but-moving on pocket is not yet validated on
own-phone data.
...
\begin{itemize}[leftmargin=1.4em]
  ...
  together.
  \item \textbf{Validation status.} Continuous Moe-Nilssen score built on PAMAP2 trunk data
  (rank-separation ROC-AUC $\approx 0.95$; no labelled accuracy by design). Chest
  $\rightarrow$ pocket placement transfer of the period selection and movement floor is
  proven on held-out MotionSense (walking $0.71$, running $0.66$, both above sedentary
  $\sim0.30$). Still owed to own-phone data: irregular-but-moving separation on pocket, and
  cycling-cadence handling.
\end{itemize}
