...
def load_shl_windows(data_dir, placement="Hips", fs=100, window_sec=5.0, overlap=0.5):
    ...
    # Load Location (GPS) if available
    loc_path = os.path.join(data_dir, "Location.txt")
    if os.path.exists(loc_path):
        # SHL Location: [T, Acc, Lat, Lon, Alt, Speed, Bearing]
        loc_df = pd.read_csv(loc_path, sep=r"\s+", header=None)
        # We only really care about speed (column 5, 0-indexed)
        data["speed"] = loc_df.iloc[:, 5].values
    else:
        data["speed"] = None
    ...
    for start in range(0, n_samples - window_samples + 1, hop_samples):
        ...
        if len(unique_labels) == 1 and unique_labels[0] in SHL_LABELS:
            ...
            win = {
                ...
                "speed": data["speed"][start:end] if data["speed"] is not None else None,
